...
cd ../..
echo -e "\n* GET KERBEROS KEYTAB GENERATED BY script/init.sh\n"
sudo cp -vi dev-local/singlestore.keytab "${KEYTAB}"
sudo chmod -v 600 "${KEYTAB}"
sudo chown -v "${USER}":"${USER}" "${KEYTAB}"

echo -e "\n* GET KERBEROS CONF\n"
sudo cp -vi build-ubuntu-example.com/services/krb5/client/krb5.conf "${KRB5_CONFIG}"
...
if [[ "${answer}" =~ ^(|y|Y)$ ]]; then
  ...
  echo "::::::::::::::
Local Keytab for singlestore
::::::::::::::"
  klist -kte /etc/singlestore.keytab
  ls -al /etc/singlestore.keytab
  echo "::::::::::::::
...
if [[ "${answer}" =~ ^(|y|Y)$ ]]; then
  if [[ -e "${KRB5CCNAME}" ]]; then
    ...
    if [[ "${answer}" =~ ^(|y|Y)$ ]]; then
      kinit -Vkt "${KEYTAB}" singlestore@EXAMPLE.COM
    fi
  else
    kinit -Vkt "${KEYTAB}" singlestore@EXAMPLE.COM
  fi
...
echo -e "\nYou can now test your dev and local Kerberos config"
